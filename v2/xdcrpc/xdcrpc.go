// xdcrpc package helps with the communication between bots and WebXDC apps via JSON-RPC 1.0
package xdcrpc
...
type StatusUpdate[T any] struct {
	...
	MaxSerial uint32 `json:"max_serial,omitempty"`
}
...
func HandleMessage(rpc *deltachat.Rpc, accId uint32, msgId uint32, serial uint32, api any) error {
...
func GetUpdates(rpc *deltachat.Rpc, accId uint32, msgId uint32, serial uint32) ([]json.RawMessage, error) {
...
func GetUpdate(rpc *deltachat.Rpc, accId uint32, msgId uint32, serial uint32) (json.RawMessage, error) {
...
func SendUpdate[T any](rpc *deltachat.Rpc, accId uint32, msgId uint32, update StatusUpdate[T], description string) error {
...
func SendPayload[T any](rpc *deltachat.Rpc, accId uint32, msgId uint32, payload T) error {
